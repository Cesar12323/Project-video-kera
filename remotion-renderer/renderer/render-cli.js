...
async function main() {
  ...
  try {
    ...
    // Point to the project root node_modules since dependencies are installed there
    const rendererNodeModules = path.resolve(__dirname, '../../node_modules');

    log(`DEBUG: Resolver node_modules path: ${rendererNodeModules}`);
    log(`DEBUG: Path exists: ${fs.existsSync(rendererNodeModules)}`);

    if (!fs.existsSync(rendererNodeModules)) {
      error(`CRITICAL: node_modules not found at ${rendererNodeModules}`);
      // Try to list content of ../../ to see what's there
      try {
        const rootDir = path.resolve(__dirname, '../../');
        log(`DEBUG: Content of ${rootDir}:`, fs.readdirSync(rootDir));
      } catch (e) { }
    }
